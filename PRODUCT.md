...
- **Verified path to actually solving the ingester's scale problem (next dedicated session, not started)**: AODP publishes a full daily MySQL dump at `albion-online-data.com/database/db_backup_<timestamp>.tgz` (path confirmed on `/developer`'s "Database table exports" section; only the last 4 daily backups are kept, so it has to be pulled once a day, not hoarded). Downloaded and inspected the current one: ~268MB compressed, ~1.94GB uncompressed, `market_history` table (`item_amount`, `silver_amount`, `item_id`, `location` [numeric cluster id, same ones `world.json` resolves], `quality`, `timestamp`, `aggregation`) covers April 1 -- today (~5.5 months), batched into only 1,731 `INSERT` statements (~1.79GB of that total) -- cheaply streamable with a regex tuple parser, no MySQL server needed. The plan: parse `market_history` from the daily dump for all 30-day volume figures (this is what `/stats/history` currently costs thousands of throttled REST calls for -- see the previous bullet and the rate-limiter fix below), keep `/stats/prices` REST calls for current prices (cheap, unchanged). This eliminates the history half of the ingester's REST traffic entirely, which is the real fix for the scale gap above -- not just a bigger rate-limit budget. Explicitly deferred to its own session rather than rushed into this one.
