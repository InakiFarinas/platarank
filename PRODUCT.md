...
- Fase 3 is live for three of its four station types: Alquimia (`/alquimia`, 174 recipes), Refinado (`/refinado`, 115 recipes: wood/fiber/ore/hide/rock -> planks/cloth/metal bars/leather/stone blocks, tiers 2-8, enchant 0-4), and Cocina (`/cocina`, 183 recipes: meals with fish/meat ingredients, same nested-enchantment recipe shape as potions). 678 priced items total. All three surfaces share one server component (`recipe-page.tsx`) and the same table/explorer/controls, branching only on `stationType` where the mechanics differ: alchemy and cocina share the farm-material fee formula (`farmStationFeePerBatch`, which also handles cocina's meat-flat/fish-tiered terms) and alchemy's fixed Brecilien-specialty assumption, while refinado has its own tier/enchant-only fee formula and, like cocina, exposes its own "especialidad" toggle instead of assuming a specific city (only alchemy's specialty city, Brecilien, is actually verified).
...
- Weapons, armor and tools are NOT built yet (rest of Fase 3's original scope) -- those need a quality dimension (5 levels) that none of the three live station types have, and per-weapon/armor-type crafting-specialty cities that haven't been verified.
